...
# 统一文件路径（你桌面的抽佣看板（剔除拼团，超客配)2.xlsx）
FILE_PATH = r"C:\Users\Administrator\Desktop\抽佣看板（剔除拼团，超客配)2.xlsx"
...
if selected_board == "商户抽佣经营看板":
    ...
    def load_commission_data():
        try:
            sheets = pd.read_excel(FILE_PATH, sheet_name=['汇总', '商户明细'], engine="openpyxl")
            st.success("✅ 抽佣数据加载成功（汇总 + 商户明细）")
            return sheets['汇总'], sheets['商户明细']
        except Exception as e:
            st.error(f"❌ 抽佣数据读取失败：{str(e)}")
            return None, None

    df_summary, df_merchant = load_commission_data()
